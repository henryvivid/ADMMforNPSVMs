...
%% datasets
name1={'votes','shuttle','skin','a9a','w8a','ijcnn1','webspam','covtype',...
...
fsave = strcat(pathsave,'NPSVMprimal_benchmark','.xls');
tname=name2(1:18);
xlswrite(fsave,tname,1);
basenum=2;

re1=[];iter_run=5;v=5;para_run=15;
for fi=1
    tic;
    name=name2{fi};
        ...
        for iter=1:iter_run
            ...
            for ip=1:K-1
                for jp=ip+1:K
                    ...
                    rho=1;epsilon=0.1;
                    %[w1,b1,w2,b2,sv]=ADMMforNPSVMdual(TXS, TYS, c, epsilon, rho);
                    [w1,b1,w2,b2,sv]=ADMMforNPSVMprimal(TXS, TYS, c, epsilon, rho);
    ...
    ler=E(acx).er;
    toc;
    time=double(toc)/(iter_run*para_run);
